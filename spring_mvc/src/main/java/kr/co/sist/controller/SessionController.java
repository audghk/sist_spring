...
public class SessionController {
	...
	public String setSession(HttpServletRequest request, HttpSession session, Model model) {
		...
		//return "forward:/index.do";//@GetMapping("/index.do")로 이동할 수 있도록 return 값 설정
	...
	public String removeSession2(SessionStatus ss) {
		...
		System.out.println(ss.isComplete());
		//return "redirect:/index.do";//redirect:로 이동하면 SessionAttribute로 설정한 세션명과 값이 QueryString으로
									  //생성되는 현상이 생길 수 있다. => 해결 redirect:를 직접 사용하지 않는다.
		return "day0610/set_session_result";
	}//removeSession2
	...
	public String getCookieAnnotation(@CookieValue(name="name",defaultValue="정명호") String name,
	        @CookieValue(name="age",defaultValue="24") int age, Model model) {
	    model.addAttribute("msg","@cookieValueAnnotation");
